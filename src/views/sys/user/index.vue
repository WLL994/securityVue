    ...
    <el-table :data="tableData" stripe style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="avatar" label="头像" width="80" align="center">
        <template #default="scope">
          <img :src="getServerUrl()+'image/userAvatar/'+scope.row.avatar" width="50" height="50" alt=""/>
<!--          {{scope.row}}-->
        </template>
      </el-table-column>
      ...
      <el-table-column prop="status" label="状态？" width="200" align="center" >
        <template  #default="scope" >
          <el-switch  v-model="scope.row.status" @change="statusChangeHandle(scope.row)" active-text="正常"
...
const handleDelete=async (id)=>{
  ...
  const res=await request.post("sys/user/delete",ids)
...
const handleResetPassword=async (id)=>{
  const res=await request.get("sys/user/resetPassword/"+id)
  if(res.data.code==200){
    ElMessage({
      type: 'success',
      message: '执行成功!'
...
const statusChangeHandle=async (row)=>{
  let res=await request.get("sys/user/updateStatus/"+row.id+"/status/"+row.status);
  if(res.data.code==200){
    ElMessage({
      type: 'success',
      message: '执行成功!'
