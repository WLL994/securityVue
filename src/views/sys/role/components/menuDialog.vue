...
const initFormData=async(id)=>{
  const res=await request.get("sys/menu/treeList");
  treeData.value=res.data.treeMenu
  form.value.id=id;

  const res2=await request.get("sys/role/menus/"+id);
  ...
  const handleConfirm=()=>{
    formRef.value.validate(async(valid)=>{
      if(valid){
        ...
        let result=await request.post("sys/role/updateMenus/"+form.value.id,menuIds);
